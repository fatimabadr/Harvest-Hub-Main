...
import { Tractor, Eye, EyeOff, Mail, Lock } from "lucide-react";
import { GoogleOAuthProvider, useGoogleLogin } from "@react-oauth/google";

const FarmerLoginContent = () => {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setError(null);
      try {
        const response = await fetch("/api/login/google/farmer", {
          method: "POST",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/login/farmer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
